...
4. A 30.18 (+/- tolerance) gap is theoretical road allowance (`L-USEC`).
5. A 20.12 (+/- tolerance) gap is surveyed road allowance (`L-SEC`).
6. LSD lines always end on the midpoint of a 20.12 boundary (`L-SEC` or `L-USEC`, whichever is the 20.12 boundary).
7. In 30.18 corridors, the 20.12 line is offset from the most western line.
...
10. When a N-S and E-W road allowance meet, the two 20.12 lines must terminate at their apparent intersection.
11. LSD lines must never enter or pass through a 20.12 road allowance (`L-SEC` or `L-USEC`).
12. LSD lines must never terminate on a 30.18 boundary when a valid 20.12 boundary exists.
13. No section/LSD blind line should pass through or inside a 20.12 (`L-SEC`) road allowance.
